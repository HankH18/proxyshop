"""FastAPI entrypoint for ``proxyshop-merchant-svc``.
...
ticket adds its routes inside the subdirectory it already owns::
    ...
    apps/merchant/svc/src/<feature>/routes.py     ->  must export a module-level `router`
...
Import namespace: ``merchant_svc`` (this directory is ``apps/merchant/svc/src``, reached through the tracked
symlink ``.pkgroot/merchant_svc`` — see the member ``pyproject.toml`` for why the layout is flat).
...
PACKAGE = __package__ or "merchant_svc"
TITLE = "proxyshop-merchant-svc"

#: This module's logger. Named ``merchant_svc.main`` by ``__name__``, which is the name an operator
