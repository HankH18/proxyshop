"""FastAPI entrypoint for ``proxyshop-trust``.
...
ticket adds its routes inside the subdirectory it already owns::
    ...
    apps/trust/src/<feature>/routes.py     ->  must export a module-level `router`
...
Import namespace: ``trust`` (this directory is ``apps/trust/src``, reached through the tracked
symlink ``.pkgroot/trust`` — see the member ``pyproject.toml`` for why the layout is flat).
...
PACKAGE = __package__ or "trust"
TITLE = "proxyshop-trust"

#: This module's logger. Named ``trust.main`` by ``__name__``, which is the name an operator
