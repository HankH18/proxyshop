...
def _record(connection: psycopg.Connection, path: Path) -> None:
    """Note that ``path`` was applied. Never consulted to decide whether to apply it."""
    with connection.transaction():
        with connection.cursor() as cur:
            cur.execute(
                f"insert into {SCHEMA_MIGRATIONS_TABLE} (filename, checksum) "
                f"values (%s, %s) "
                f"on conflict (filename) do update set checksum = excluded.checksum, "
                f"applied_at = now()",  # noqa: S608 - table name is a module constant
