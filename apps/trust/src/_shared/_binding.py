...
class objects. That is not a theoretical worry in this service: ``replay()`` calls the
...
def publish(package_name: str, spellings: Iterable[str], sub_name: str, module: ModuleType) -> None:
    ...
    * the ``sys.modules`` entry is what makes ``from apps.trust.src.<feature>.<module> import
      X`` a cache hit rather than a second execution of the file;
    * the **attribute** on the package object is what makes plain ``import
      apps.trust.src.<feature>.<module>`` followed by ``....<module>.X`` work, which a
      ``sys.modules`` entry alone does NOT give you.
...
def _import_alternative_spelling(spelling: str, root: Path | None) -> bool:
    ...
    ``trust.<feature>`` but not ``apps.trust.src.<feature>``, and without the fallback the second
