"""Scaffold smoke test for ``apps/trust``.
...
def test_import_namespace_resolves() -> None:
    """``trust`` imports, and resolves to this member's FLAT ``src/`` directory."""
    module = importlib.import_module("trust")
    assert module.__file__ is not None
    resolved = Path(module.__file__).resolve().parent
    assert resolved == (REPO_ROOT / "apps/trust/src").resolve()
...
def test_app_starts_with_the_routers_it_discovers() -> None:
    ...
    assert app.openapi()["info"]["title"] == "proxyshop-trust"


def test_router_discovery_is_a_glob_not_a_hard_coded_list() -> None:
    """Every discovered name is ``trust.<feature>.routes`` — the contract workers rely on."""
    main = importlib.import_module("trust.main")
    for name in main.discover_router_modules():
        assert name.startswith("trust.")
