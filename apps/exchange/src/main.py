"""FastAPI entrypoint for ``proxyshop-exchange``.
...
ticket adds its routes inside the subdirectory it already owns::
    ...
    apps/exchange/src/<feature>/routes.py     ->  must export a module-level `router`
...
Import namespace: ``exchange`` (this directory is ``apps/exchange/src``, reached through the tracked
symlink ``.pkgroot/exchange`` — see the member ``pyproject.toml`` for why the layout is flat).
...
PACKAGE = __package__ or "exchange"
TITLE = "proxyshop-exchange"

#: This module's logger. Named ``exchange.main`` by ``__name__``, which is the name an operator
