"""Scaffold smoke test for ``apps/exchange``.
...
def test_import_namespace_resolves() -> None:
    """``exchange`` imports, and resolves to this member's FLAT ``src/`` directory."""
    module = importlib.import_module("exchange")
    assert module.__file__ is not None
    resolved = Path(module.__file__).resolve().parent
    assert resolved == (REPO_ROOT / "apps/exchange/src").resolve()
...
def test_app_starts_with_the_routers_it_discovers() -> None:
    ...
    """
    main = importlib.import_module("exchange.main")
    app = main.create_app()
    assert isinstance(app, FastAPI)
    assert isinstance(app.state.mounted_routers, list)
    assert app.openapi()["info"]["title"] == "proxyshop-exchange"


def test_router_discovery_is_a_glob_not_a_hard_coded_list() -> None:
    """Every discovered name is ``exchange.<feature>.routes`` — the contract workers rely on."""
    main = importlib.import_module("exchange.main")
    for name in main.discover_router_modules():
        assert name.startswith("exchange.")
