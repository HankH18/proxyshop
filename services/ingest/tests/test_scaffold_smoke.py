"""Scaffold smoke test for ``services/ingest``.
...
def test_import_namespace_resolves() -> None:
    """``ingest`` imports, and resolves to this member's FLAT ``src/`` directory."""
    module = importlib.import_module("ingest")
    assert module.__file__ is not None
    resolved = Path(module.__file__).resolve().parent
    assert resolved == (REPO_ROOT / "services/ingest/src").resolve()
...
def test_app_starts_with_the_routers_it_discovers() -> None:
    ...
    assert app.openapi()["info"]["title"] == "proxyshop-ingest"


def test_router_discovery_is_a_glob_not_a_hard_coded_list() -> None:
    """Every discovered name is ``ingest.<feature>.routes`` — the contract workers rely on."""
    main = importlib.import_module("ingest.main")
