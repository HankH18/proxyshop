"""FastAPI entrypoint for ``proxyshop-ingest``.
...
ticket adds its routes inside the subdirectory it already owns::
    ...
    services/ingest/src/<feature>/routes.py     ->  must export a module-level `router`
...
Import namespace: ``ingest`` (this directory is ``services/ingest/src``, reached through the tracked
symlink ``.pkgroot/ingest`` — see the member ``pyproject.toml`` for why the layout is flat).
...
PACKAGE = __package__ or "ingest"
TITLE = "proxyshop-ingest"

#: This module's logger. Named ``ingest.main`` by ``__name__``, which is the name an operator
