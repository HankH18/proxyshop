"""Scaffold smoke test for ``services/shopify-stub``.
...
def test_import_namespace_resolves() -> None:
    """``shopify_stub`` imports, and resolves to this member's FLAT ``src/`` directory."""
    module = importlib.import_module("shopify_stub")
    assert module.__file__ is not None
    resolved = Path(module.__file__).resolve().parent
    assert resolved == (REPO_ROOT / "services/shopify-stub/src").resolve()


def test_scope_directories_exist() -> None:
    """Every directory a ticket scope names is present, so `pytest <path>` cannot exit 4."""
    for relative in ["services/shopify-stub/fixtures/recorded"]:
