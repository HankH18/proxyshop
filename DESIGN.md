...
- Rank formula (single, published, jointly weighted — **this is the only rank formula in the system**): eligibility filters first (blacklist fail-closed, expiry, valid checkout domain, hard constraints per R19), then `rank_score = w_m*intent_match + w_e*verified_claim_ratio + w_t*trust + w_v*price_value + w_d*delivery_fit − policy_penalties`, features normalized to [0,1], stable tie-break (verified hard-fit count → trust → price → bid id), component map + human-readable explanation returned. Evidence freshness lives in verification confidence, not the formula. Bandit adjusts exposure/exploration only.
  ...
  - `delivery_fit` reads a **credible** delivery estimate, not a declared one (D57). The store's quoted `Offer.delivery_estimate_days` is divided by its `shipped_on_time` posterior (`alpha/(alpha+beta)` off the trust snapshot's `TrustDims`, clamped at a floor so the inflation is bounded), and those adjusted quotes are then compared against each other across the offers this auction received — all quoting for the same `Intent.ship_to`, which is what makes comparing them meaningful and comparing them across auctions meaningless. A store with too little observed dispatch history has **no admitted promise**: the feature is absent for it, not good and not bad. Absent — no quote, no admissible record, fewer than two admissible quotes in the auction, or a degenerate range — is **0.5 (neutral), never 0**. A promise must be earned before it counts: while the feed was the declared number alone, a store bought up to `w_d` of the score by typing a smaller one.
